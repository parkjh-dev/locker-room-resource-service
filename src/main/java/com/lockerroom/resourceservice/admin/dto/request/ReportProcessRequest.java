...
public record ReportProcessRequest(
        @Schema(description = "처리 결과 상태 (PROCESSED=조치 완료, DISMISSED=기각)", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull ReportStatus status,

        @Schema(description = "조치 종류 (DELETE_POST, SUSPEND_USER 등). status=PROCESSED일 때 필요.", nullable = true)
        ReportAction action,
